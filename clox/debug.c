...
{
    uint16_t jump_offset = (uint16_t)(chunk->code[offset + 1] << 8);
    jump_offset |= chunk->code[offset + 2];
    printf("%-16s %4d -> %d\n", name, offset, offset + 3 + sign * jump_offset);
    return offset + 3;
}
...
int disassembleInstruction(Chunk *chunk, int offset)
{
    ...
    case OP_CLOSURE: {
        offset++;
        uint8_t constant_index = chunk->code[offset++];
        printf("%-16s %4d ", "OP_CLOSURE", constant_index);
        printValue(chunk->constants.values[constant_index]);
        printf("\n");
        ObjFunction *function = AS_FUNCTION(chunk->constants.values[constant_index]);
